...
A phenotyping session is what we have to convene when new data is added to our existing dataset. Decisions made at a phenotyping session allow the lab to categorize BioBank phenotypes as binary or quantitative (or a combination of both), and subsequently allow the lab to run a number of downstream analyses including GWAS and PheWAS.
...
- In the case of it being a binary trait: your decisions are pretty simple. Find out what value corresponds to cases and what value corresponds to controls via the BioBank search box above.
...
- In the case of it being a quantitative trait, we need to look at how it is encoded. Some quantitative traits are encoded just as is, like [height](http://biobank.ctsu.ox.ac.uk/crystal/field.cgi?id=12144), which is encoded in the BioBank as cm. If this is the case, leave all fields except name and field ID blank.
  - Some multiple-trait fields are quantitative. Imagine that you have a question like the following:
    ...
    For this question, you should split into 6 rows. The naming convention for these kinds of rows is "Field Description (Parenthetical additional descriptor)" (e.g. "Average amount of sleep (0-2 hours)"). Additionally, there is an "ORDER" column in the sheet. For this kind of quantitative trait, the person doing this trait should ordinally assign values to this column based on quantity (1 for "0-2 hours", 2 for "2-4 hours", etc.).
