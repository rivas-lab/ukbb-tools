...
if [ -s ${metal_dir}/${GBE_ID}.metal.tsv.gz ] && [ -s ${metal_dir}/${GBE_ID}.metal.info.txt ] ; then
...
echo ${pops_str} | tr ',' '\n' | while read pop ; do
    sumstats_l=${ukb_data_dir}/${geno_data}/gwas/current/${pop}/ukb24983_v2_hg38.${GBE_ID}.exome-spb.$(get_plink_suffix ${GBE_ID}).gz
...
if [ "${n_in_files}" -gt 0 ] ; then
    bash ${metal_script} --assembly hg38 -o ${metal_dir}/${GBE_ID} -f ${in_file_list}
else
...
#######################
usage:
ml load R/3.6 gcc/6 # or your favorite R env
