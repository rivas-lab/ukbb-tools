#!/bin/bash
#SBATCH --job-name=mrpmm_array_qt
#SBATCH --output=mrpmm_logs/mrpmm_array_qt.%A_%a.out
#SBATCH --nodes=1
#SBATCH --cores=8
#SBATCH --mem=200000
...
# define functions
usage () {
    echo "$0: MRPMM script for QT phenotype array data"
...
start_idx=$1
output_folder="/oak/stanford/groups/mrivas/users/guhan/sandbox/mrpmm_array"
this_idx=$_SLURM_ARRAY_TASK_ID

# read in the actual thing
GBE_ID=$(awk -F'\t' '{print $1}' mrparrayresults.tsv | tail -n +2 | egrep -v cancer | egrep -v BIN_FC | egrep -v FH | egrep -v HC | egrep -v BIN | egrep -v TTE | sort -u | awk -v start_idx=$start_idx -v this_idx=$this_idx 'NR==(start_idx + this_idx - 1) {print $1}')

grep $GBE_ID mrparrayresults.tsv | awk -F'\t' '{print $5}' > $output_folder/${GBE_ID}_genes.tsv
...
POP="metal"
PATH_TO_FILE=$(find /oak/stanford/groups/mrivas/ukbb24983/array-combined/metal/20200717 -name "$GBE_ID.*gz" | grep -v freeze | grep -v old | grep -v ldsc | grep $POP | grep -v plink)
...
for gene in $(cat $output_folder/${GBE_ID}_genes.tsv); do
    awk -F'\t' -v gene=$gene '{if ($4 == gene) {print $1}}' ../13_mrp/ref/ukb_cal-consequence_wb_maf_gene_ld_indep_mpc_pli.tsv > $output_folder/${GBE_ID}_${gene}_variants.tsv
    sed -i '1s/^/V\n/' $output_folder/${GBE_ID}_${gene}_variants.tsv
    python3 mrpmm.py --variants $output_folder/${GBE_ID}_${gene}_variants.tsv --phenotypes $output_folder/$GBE_ID.tmp.txt --metadata_path ../13_mrp/ref/ukb_cal-consequence_wb_maf_gene_ld_indep_mpc_pli.tsv --out_folder $output_folder --C 1 2 3 4 5 --se_thresh 100 --maf_thresh 0.01 --variant_filters ptv pav
