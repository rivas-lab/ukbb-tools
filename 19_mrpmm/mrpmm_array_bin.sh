#!/bin/bash
#SBATCH --job-name=mrpmm_array_bin
#SBATCH --output=mrpmm_logs/mrpmm_array_bin.%A_%a.out
#SBATCH --nodes=1
#SBATCH --cores=8
#SBATCH --mem=200000
...
# define functions
usage () {
    echo "$0: MRPMM script for BIN phenotype array data"
...
GBE_ID=$(awk -F'\t' '{print $1}' mrparrayresults.tsv | tail -n +2 | egrep -v QT_FC | egrep -v INI | sort -u | awk -v start_idx=$start_idx -v this_idx=$this_idx 'NR==(start_idx + this_idx - 1) {print $1}')
...
for gene in $(cat $output_folder/${GBE_ID}_genes.tsv); do
    ...
    python3 mrpmm.py --variants $output_folder/${GBE_ID}_${gene}_variants.tsv --phenotypes $output_folder/$GBE_ID.tmp.txt --metadata_path ../13_mrp/ref/ukb_cal-consequence_wb_maf_gene_ld_indep_mpc_pli.tsv --out_folder $output_folder --C 1 2 3 4 5 --se_thresh 0.2 --maf_thresh 0.01 --variant_filters ptv pav
