#!/bin/bash
#SBATCH --job-name=mrpmm_exome_qt
#SBATCH --output=mrpmm_logs/mrpmm_exome_qt.%A_%a.out
#SBATCH --nodes=1
#SBATCH --cores=8
#SBATCH --mem=200000
...
# define functions
usage () {
    echo "$0: MRPMM script for QT phenotype exome data"
...
for gene in $(cat $output_folder/${GBE_ID}_genes.tsv); do
    ...
    python3 mrpmm.py --variants $output_folder/${GBE_ID}_${gene}_variants.tsv --phenotypes $output_folder/$GBE_ID.tmp.txt --metadata_path ../13_mrp/ref/ukb_exm_oqfe-consequence_wb_maf_gene_ld_indep_mpc_pli.tsv --out_folder $output_folder --C 1 2 3 4 5 --se_thresh 100 --maf_thresh 0.01 --variant_filters ptv pav
