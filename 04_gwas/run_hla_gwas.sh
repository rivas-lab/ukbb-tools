#!/bin/bash
#SBATCH --job-name=RL_HLA
#SBATCH --output=rerun_logs/run_hla.%A_%a.out
#SBATCH  --error=rerun_logs/run_hla.%A_%a.err
#SBATCH --nodes=1
#SBATCH --cores=1
#SBATCH --mem=4000
...
usage () {
    ...
    echo "usage: sbatch --array=1-<number of array jobs> $0 n_jobs (inclusive.)"
    echo ''
...
# iterate and run
for phe_path in $(cat ../05_gbe/phenotype_info.tsv | awk -v min_N=${min_N_count} 'NR > 1 && $7 >= min_N' | awk -v idx=$this_idx -v mod=$n_jobs '((NR % mod)==idx) {print $NF}' ); do 
    gbeId=$(basename $phe_path | awk '{gsub(".phe","");print}')

    # run gwas with default GBE parameters
    pop="white_british"
    gwasOutDir=$(echo $(dirname $(dirname $phe_path)) | awk '{gsub("phenotypedata","hla/gwas"); print}')/${pop}
    if [ ! -d ${gwasOutDir}/logs ] ; then mkdir -p ${gwasOutDir}/logs ; fi
    if [ ! -d $log_dir ] ; then mkdir -p $log_dir ; fi

    python gwas.py --run-hla --run-now --memory $mem --cores $cores --pheno $phe_path --out $gwasOutDir --population $pop --log-dir $log_dir
