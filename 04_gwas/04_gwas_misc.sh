...
GBE_ID_to_gwas_basename () {
    ...
    local gwas_type=$2
    local prefix=$3
    echo ${prefix}.${GBE_ID}.${gwas_type}.glm
#    ukb24983_v2_hg19.BIN10030500.genotyped.glm.logistic.hybrid.gz
}

get_sumstats_name_array () {
    local phe_file=$1
    local population=$2
    local prefix=$3

    dir=$(phe_path_to_gwas_path $(dirname $phe_file) $population "cal")
    basename_prefix=$(GBE_ID_to_gwas_basename $(basename ${phe_file%.phe}) "genotyped" ${prefix})
    sumstats_file=$(find ${dir} -type f -name "${basename_prefix}*")
    
    echo ${sumstats_file}
}

check_sumstats_array () {
    local phe_file=$1
    local population=$2
    local prefix=$3

    sumstats_file="$(get_sumstats_name_array ${phe_file} ${population} ${prefix})"
...
check_sumstats_array_wrapper () {
    ...
    echo "$idx $phe_file $(check_sumstats_array $phe_file $population $prefix)" | tr " " "\t"
}
