...
#SBATCH --output=rerun_logs_exome2/run_exome.%A_%a.out
#SBATCH  --error=rerun_logs_exome2/run_exome.%A_%a.err
#SBATCH --nodes=1
#SBATCH --cores=4
#SBATCH --mem=25600
...
phe_path=$(cat check_exome_gwas.42157287.out | awk '(NR>1 && $3 == "NA"){print $2}' | awk -v start_idx=$start_idx -v this_idx=$this_idx 'NR==(start_idx + this_idx - 1) {print $NF}')
