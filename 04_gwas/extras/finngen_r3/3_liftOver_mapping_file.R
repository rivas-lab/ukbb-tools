...
# output
out_f               <- file.path(finngen_dir, 'finngen_r3_variants.master.tsv.gz')
...
finngen_vars_hg19_df <- fread(finngen_vars_hg19_f, select=c('ID', 'FASTA_REF'), colClasses=c('ID'='character', 'FASTA_REF'='character'))
...
select(CHROM, POS, REF, ALT, ID, hg19_REF_ALT_flip, hg38_REF, hg38_ALT, hg38_ID, rsids, nearest_genes) %>%
rename('#CHROM' = 'CHROM') %>%
