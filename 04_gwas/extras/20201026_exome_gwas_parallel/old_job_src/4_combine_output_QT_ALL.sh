...
if [ "${batch_idx}" -eq 1570 ] ; then
    # log file
    template_f=${data_d}/${pop}-batch/logs/ukb24983_exomeOQFE.batch__BATCH__.QT_ALL.glm.log
    ...
    template_f=${data_d}/${pop}-batch/ukb24983_exomeOQFE.batch__BATCH__.${trait}.glm.linear
...
exit 0
########################################################################
bash 4_combine_output_QT_ALL.sh 1 non_british_white

sbatch -p mrivas,normal \
    --time=2:0:00 --mem=6000 --nodes=1 --cores=1 \
    ...
    --array=1-314 ${parallel_sbatch_no_err_check_sh} \
    4_combine_output_QT_ALL.sh ${parallel_idx} 5 non_british_white
