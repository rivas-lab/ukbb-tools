...
zcat /oak/stanford/groups/mrivas/users/guhan/sandbox/mrp_rv_ma_exome/white_british_BIN10030820_gene_maf_0.01_se_0.2.tsv.gz | awk -F'\t' '{if (NR == 1) {print "#GBE_ID\tGBE_short_name\tpops\tnum_pops\t"$0}}' > mrp_rv_ma_exome_singlepop_gbe.tsv
...
for file in $(ls $dir | grep "0\.0005" | head -1); do
    zcat $dir/$file | awk -F'\t' '{if (NR == 1) {print "#GBE_ID\tGBE_short_name\tpops\tnum_pops\t"$0}}' > mrp_rv_ma_exome_ultrarare_multipop_gbe.tsv
done

for phen in $(cut -f2 ../05_gbe/exome/200k/icdinfo.shortnames.exome.tsv | tail -n +2 | sort -u); do
    if ! grep -Fxq "$phen" gbe_blacklist.tsv; then
        shortname=$(awk -F'\t' -v gbe="$phen" '{if ($2 == gbe) {print $5}}' ../05_gbe/exome/200k/icdinfo.shortnames.exome.tsv)
        file=$(find $dir -name "*_${phen}_*" | grep "0\.0005")
        numpops=0
        if [ -f "$file" ]; then
            pops=$(echo $(basename $file) | awk -F"_$phen" '{print $1}')
            for pop in white_british non_british_white e_asian s_asian african related others; do
            ...
            if [ $numpops -gt 1 ]; then
                zcat $file | awk -F'\t' -v filename="$phen" -v pops="$pops" -v numpops="$numpops" -v shortname="$shortname" '{if (NR != 1) {print filename"\t"shortname"\t"pops"\t"numpops"\t"$0}}' >> mrp_rv_ma_exome_ultrarare_multipop_gbe.tsv
            fi
        fi
    fi
done

zcat /oak/stanford/groups/mrivas/users/guhan/sandbox/mrp_rv_ma_exome/white_british_HC36_gene_maf_0.0005_se_0.2.tsv.gz | awk -F'\t' '{if (NR == 1) {print "#GBE_ID\tGBE_short_name\tpops\tnum_pops\t"$0}}' > mrp_rv_ma_exome_ultrarare_singlepop_gbe.tsv
...
for phen in $(cut -f1 ../05_gbe/exome/200k/exome_phenotype_info.tsv | tail -n +2 | sort -u); do
    if ! grep -Fxq "$phen" gbe_blacklist.tsv; then
        ...
        if [ -f "$file" ]; then
            ...
            if [ $numpops -eq 1 ]; then
                zcat $file | awk -F'\t' -v filename="$phen" -v pops="$pops" -v numpops="$numpops" -v shortname="$shortname" '{if (NR != 1) {print filename"\t"shortname"\t"pops"\t"numpops"\t"$0}}' >> mrp_rv_ma_exome_ultrarare_singlepop_gbe.tsv
            fi
        fi
    fi
done
...
bgzip -f mrp_rv_ma_exome_ultrarare_gbe.tsv
bgzip -f biomarkers_mrp_rv_ma_exome_ultrarare_gbe.tsv
