...
zcat /oak/stanford/groups/mrivas/users/guhan/sandbox/mrp_rv_ma_array/african_non_british_white_others_related_s_asian_white_british_INI10030500_gene_maf_0.01_se_100.0.tsv.gz | awk -F'\t' '{if (NR == 1) {print "#GBE_ID\tGBE_short_name\tpops\tnum_pops\t"$0}}' > biomarkers_mrp_rv_ma_array_gbe.tsv
for phen in $(grep adjusted ../05_gbe/array-combined/phenotype_info.tsv | cut -f1 | grep -v BIN | sort -u); do
    shortname=$(awk -F'\t' -v gbe="$phen" '{if ($2 == gbe) {print $5}}' ../05_gbe/array-combined/icdinfo.shortnames.tsv)
    ...
    if [ -f "$file" ]; then
        ...
        if [ $numpops -gt 1 ]; then
            zcat $file | awk -F'\t' -v filename="$phen" -v pops="$pops" -v numpops="$numpops" -v shortname="$shortname" '{if (NR != 1) {print filename"\t"shortname"\t"pops"\t"numpops"\t"$0}}' >> biomarkers_mrp_rv_ma_array_gbe.tsv
        fi
    fi
done
