...
min_N_count=100
GBE_ID=$(cat ../05_gbe/exome_phenotype_info.tsv | awk -v min_N=${min_N_count} 'NR > 1 && $8 >= min_N' | egrep -v MED | grep INI30140 | awk -v start_idx=$start_idx -v this_idx=$this_idx 'NR==(start_idx + this_idx - 1) {print $1}' )
POP="white_british"
echo $GBE_ID >&1
