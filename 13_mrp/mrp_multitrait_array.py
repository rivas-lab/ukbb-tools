...
qc = pd.read_table('/oak/stanford/groups/mrivas/ukbb24983/array-combined/gwas/current/gwas.qc-SE02.tsv')
...
for cluster in clusters['cluster'].unique():
    ...
    for i, row in cluster_df.iterrows():
        ...
        pops = ['white_british']
        for pop in pops:
            stream = os.popen('find /oak/stanford/groups/mrivas/ukbb24983/array-combined/gwas -name "*.' + pheno + '.*gz" | grep -v freeze | grep -v old | grep -v ldsc | grep current | grep ' + pop)
    ...
    stream = os.popen('sbatch -t 02-00:00:00 -p normal,owners,mrivas --mem=200000 --nodes=1 --cores=16 --output=mrp_logs/mrp_multitrait_array.%j.out --wrap="/share/software/user/open/python/3.6.1/bin/python3 mrp_production.py --file ' + input_folder + str(cluster) + '.txt --R_var independent similar --variants ptv pav --sigma_m_types sigma_m_mpc_pli --filter_ld_indep --se_thresh 100 --maf_thresh 0.01 0.0005 --metadata_path /oak/stanford/groups/mrivas/ukbb24983/cal/pgen/ukb_cal-consequence_wb_maf_gene_ld_indep_mpc_pli.tsv --out_folder /oak/stanford/groups/mrivas/users/guhan/sandbox/mrp_multitrait_array/output/ --out_filename ' + str(cluster) + '"')
