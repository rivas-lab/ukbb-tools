...
results = pd.read_table('mrp_rv_ma_exome_gbe.txt')
...
shortnames = pd.read_table('../05_gbe/extras/20200812_GBE_category/GBE_category.20201024.tsv')
...
for name, df in zip(['pav', 'ptv'], [pav, ptv]):
    df = df.merge(shortnames, how='left', left_on='#GBE_ID', right_on='GBE_ID')
    ...
    df = df[['LOCUS_ID', '#GBE_category', 'GBE_short_name', 'CHROM', 'POS', 'MARKER', 'gene']]
    df.columns = ['LOCUS_ID', 'CATEGORY', 'TRAIT', 'CHR', 'BP', 'MARKER', 'GENE']
    df.to_csv(name + '_fuji.tsv', sep='\t', index=False)
    color_df = df[['CATEGORY', 'TRAIT']].drop_duplicates()
